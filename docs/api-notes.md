...
**Write / authoring (verified live with `ENABLE_WRITE_OPS=true`):**

- `create_flow` → `POST .../flows` body `{ properties: { displayName, state, definition, connectionReferences } }` → **201 Created**. A minimal connection-free flow (Recurrence → Compose) works; returns the created flow.
- `update_flow` → `PATCH .../flows/{flow}` body `{ properties: {…only changed fields} }` → **200**. Verified editing the definition (trigger recurrence Day/1 → Week/2, action inputs) and renaming.
- `delete_flow` → `DELETE .../flows/{flow}` → **200 with an EMPTY body** (not 204). This exposed a client bug — `client.ts` now treats any empty 2xx body as success (not just 204), which also covers `start`/`stop`/`cancel`/`resubmit` likely returning empty 200s.
- Note: this makes the server **management + light authoring**, beyond the original management-only scope (create/update/delete were v2 candidates).
